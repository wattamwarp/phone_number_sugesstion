...
import com.example.phone_number_sugesstion.Response
...
class MainActivity : FlutterPlugin, MethodCallHandler, ActivityAware, ActivityResultListener {
    ...
    var response:Response=    Response(statusCode = 0, phoneNumber = "", errorMessage = "")
    ...
    private fun onHintRequest(resultCode: Int, data: Intent?) {
        if (resultCode == RESULT_OK && data != null) {
            try {
                ...
                var response = Response(statusCode = 200, phoneNumber = phoneNumber, errorMessage = "")
                ignoreIllegalState { pendingResult?.success(response.toJson()) }
                return
            } catch (e: Exception) {
                var errorMsg:String=""
                response = Response(statusCode = 404, phoneNumber = "", errorMessage = errorMsg)
